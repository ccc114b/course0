# 系統程式

本課程介紹電腦系統的底層運作。

## 1. 程式語言
* [C語言](../../wiki/系統程式/C語言.md)
* [組合語言](../../wiki/系統程式/組合語言.md)
* [函數式編程](../../wiki/系統程式/函數式編程.md)

## 2. 編譯器與直譯器
* [編譯器](../../wiki/系統程式/編譯器.md)
    * 程式： [系統程式/compiler/](../../code/系統程式/compiler/) - 編譯器實作
* [直譯器](../../wiki/系統程式/直譯器.md)
    * 程式： [系統程式/interpreter/](../../code/系統程式/interpreter/) - 直譯器實作
* [管線處理與編譯器](../../wiki/系統程式/管線處理與編譯器.md)

## 3. 虛擬機器與中介表示
* [虛擬機](../../wiki/系統程式/虛擬機.md)
    * 程式： [系統程式/irvm/](../../code/系統程式/irvm/) - IRVM 實作
* [圖靈機](../../wiki/系統程式/圖靈機.md)
    * 程式： [演算法/theory/turingMachine/](../../code/演算法/theory/turingMachine/) - 圖靈機實作
* [有限狀態機](../../wiki/系統程式/有限狀態機.md)
    * 程式： [演算法/theory/finiteStateMachine/](../../code/演算法/theory/finiteStateMachine/) - FSM 實作
* [形式文法](../../wiki/系統程式/形式文法.md)
    * 程式： [演算法/theory/grammar/](../../code/演算法/theory/grammar/) - 文法實作

## 4. 作業系統
* [作業系統](../../wiki/系統程式/作業系統.md)
    * 程式： [系統程式/os/](../../code/系統程式/os/) - OS 專案
* [行程與執行緒](../../wiki/系統程式/行程與執行緒.md)
* [排程](../../wiki/系統程式/排程.md)
* [虛擬記憶體](../../wiki/系統程式/虛擬記憶體.md)
* [記憶體管理](../../wiki/系統程式/記憶體管理.md)
* [檔案系統](../../wiki/系統程式/檔案系統.md)
* [垃圾搜集](../../wiki/系統程式/垃圾搜集.md)

## 5. 網路
* [網路](../../wiki/系統程式/網路.md)
    * 程式： [系統程式/network/](../../code/系統程式/network/) - 網路程式
* [HTTP](../../wiki/系統程式/HTTP.md)

## 6. 密碼學與安全
* [密碼學與資訊安全](../../wiki/系統程式/密碼學與資訊安全.md)
    * 程式： [系統程式/crpyto/](../../code/系統程式/crpyto/) - 密碼學實作
* [區塊鏈與比特幣](../../wiki/系統程式/區塊鏈與比特幣.md)
    * 程式： [系統程式/textdata/blockchain0/](../../code/系統程式/textdata/blockchain0/) - 區塊鏈

## 7. 並行與效能
* [並行計算與異構平台](../../wiki/系統程式/並行計算與異構平台.md)
* [同步與通訊](../../wiki/系統程式/同步與通訊.md)
* [訊號處理](../../wiki/系統程式/訊號處理.md)

## 8. 媒體處理
* 影片編碼
    * 程式： [系統程式/media/](../../code/系統程式/media/) - 媒體處理

## 9. 連結與載入
* [連結器與載入器](../../wiki/系統程式/連結器與載入器.md)

## 10. 開發工具
* [GDB_LLDB](../../wiki/系統程式/GDB_LLDB.md)