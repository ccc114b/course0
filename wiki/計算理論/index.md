# 計算理論

計算理論探討計算的本質、能力與極限，是電腦科學的理論基礎。

## 概念

- [計算理論](計算理論.md) - 可計算性與複雜度理論 | tags: P vs NP, NPC
- [NP完全性](NP完全性.md) - NP完全問題與P vs NP | tags: SAT, 歸約, 複雜度
- [圖靈機](圖靈機.md) - 通用計算模型 | tags: 停機問題, Church-Turing論點
- [有限狀態機](有限狀態機.md) - 正則語言與自動機 | tags: DFA, NFA
- [形式文法](形式文法.md) - 描述語言語法的數學工具 | tags: Chomsky階層
- [Lambda演算](Lambda演算.md) - 函數式計算理論 | tags: Church編碼, Y_Combinator
- [自動機理論](自動機理論.md) - 形式語言與計算模型 | tags: 正則表達式
- [資訊理論](資訊理論.md) - 資訊量的數學理論 | tags: 熵, 壓縮
- [Noam_Chomsky](Noam_Chomsky.md) - 語言學家與認知科學家 | tags: 轉換生成語法

## 主題

- [函數式編程](函數式編程.md) - 以函數為主的程式設計範式 | tags: 高階函數, 不可變性

## 日誌

- 參見 [log.md](log.md)