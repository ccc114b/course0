...
class Line:
    ...
    def __repr__(self) -> str:
        return f"Line({self._point}, {self._direction})"
    ...
    def is_parallel_to(self, other: "Line", tol: float = 1e-9) -> bool:
        """Check if lines are parallel."""
        if not isinstance(other, Line):
            return False
        return self._direction.is_parallel(other._direction, tol)

    def is_perpendicular_to(self, other: "Line", tol: float = 1e-9) -> bool:
