...
class TestIntegration(unittest.TestCase):
    def test_cli_index_builds(self):
        if os.path.exists(os.path.join(TEST_DIR, IDX_PATH)):
            os.remove(os.path.join(TEST_DIR, IDX_PATH))
        result = subprocess.run(["./index", CORPUS], capture_output=True, cwd=TEST_DIR)
        self.assertEqual(result.returncode, 0)
        self.assertTrue(os.path.exists(os.path.join(TEST_DIR, IDX_PATH)))

    def test_cli_query_works(self):
        subprocess.run(["make", "query"], capture_output=True, cwd=TEST_DIR)
        result = subprocess.run(["./query", "system"], capture_output=True, text=True, cwd=TEST_DIR)
        self.assertIn("system", result.stdout.lower())
